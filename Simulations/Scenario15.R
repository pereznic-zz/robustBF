...
# Generating the outlier datasets
normalDatasets<-genNormData(betas,x_correlations,n_predictors,n_obs,n_datasets,r_squared)
lowXhighYDatasets<-outliers_XY(normalDatasets,n_outliers,coeffs,lowHigh = TRUE,minValIQR,maxValIQR)

# set data to use
currentData<-lowXhighYDatasets
...
write.csv(LR_PostProbs,"Scenario15_LR_Probs.csv")
write.csv(RLM_PostProbs,"Scenario15_RLM_Probs.csv")
write.csv(LMROB_PostProbs,"Scenario15_LMROB_Probs.csv")
write.csv(LR_results[[1]],"Scenario15_LR_Betas.csv")
write.csv(RLM_results[[1]],"Scenario15_RLM_Betas.csv")
write.csv(LMROB_results[[1]],"Scenario15_LMROB_Betas.csv")
